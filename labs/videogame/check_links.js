...
const checkAll = async () => {
    ...
    for (const [id, title, cover] of rules) {
        const romUrl = S + id;
        const imgUrl = cover ? B + cover : null;

        const romCheck = await checkUrl(romUrl);
        if (romCheck.status !== 200) {
            console.log(`[ROM FAIL] ${title} (${id}): ${romCheck.status}`);
        } else {
            // console.log(`[ROM OK] ${title}`);
        }

        if (imgUrl) {
            const imgCheck = await checkUrl(imgUrl);
            if (imgCheck.status !== 200) {
                console.log(`[IMG FAIL] ${title} (${cover}): ${imgCheck.status}`);
            }
        } else {
            console.log(`[IMG MISSING] ${title} has no cover defined.`);
        }
    }
    console.log("Done.");
};
