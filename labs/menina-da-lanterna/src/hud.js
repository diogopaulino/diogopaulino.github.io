...
export class Hud {
    ...
    setStealth(alert) {
        ...
        this.el.vignette.style.opacity = String(Math.min(0.62, alert * 0.7));
    }

    setFuel(n) {
        const p = Math.max(0, Math.min(100, n));
        this.el.fuelFill.style.transform = `scaleX(${p / 100})`;
        this.el.fuelValue.textContent = `${Math.round(p)}`;
        this.el.fuelFill.closest('.fuel').dataset.low = p < 22 ? 'true' : 'false';
    }

    setMemories(n, total) {
        this.el.memories.textContent = `${n}/${total}`;
    }
