...
/** Suspensório do chef: a tira é estreita e a lingueta de baixo abre. */
const CHEF_SUSPENDER = (() => {
    const p = [
        [-0.022, 0.18],
        [0.022, 0.18],
        [0.02, 0.02],
...
/** Alça do explorador: as pontas ficam finas e o meio sobe num passador. */
const EXPLORER_STRAP = (() => {
    const p = [
        [-0.08, -0.012],
        [-0.04, -0.014],
        [-0.02, -0.028],
...
const BODIES = {
    ...
    chef: (kit) => build(kit, {
        ...
        detail: ({ add, mats }) => {
            ...
            for (const x of [-0.1, 0.1]) {
                const strap = add.mesh(CHEF_SUSPENDER, mats.dark, [x, 0.86, 0.19]);
                strap.name = 'chefSuspender';
            }
    ...
    explorer: (kit) => build(kit, {
        ...
        detail: ({ add, mats }) => {
            ...
            const strap = add.mesh(EXPLORER_STRAP, mats.primary, [0, 0.98, 0.18]);
            strap.name = 'explorerStrap';
...
/**
 * Crachá do cientista: a barra de baixo abre em duas pontas.
 * O vão entre elas recua cerca de 10 cm.
 */
const LAB_BADGE = (() => {
    const p = [
        [-0.08, 0.05],
