...
export class Input {
    ...
    consumeShutter() {
        const v = this.shutterPressed;
        this.shutterPressed = false;
        return v;
    }

    get photoMode() {
        return this.photoHeld || this.rightDown || this.keys.has('photoHold');
    }
    ...
    bindTouch({ stick, knob, look, photo, shutter }) {
        ...
        if (photo) {
            photo.addEventListener('touchstart', (ev) => {
                ev.preventDefault();
                this.photoHeld = true;
            }, { passive: false });
            photo.addEventListener('touchend', () => { this.photoHeld = false; });
        }
        if (shutter) {
            shutter.addEventListener('touchstart', (ev) => {
                ev.preventDefault();
    ...
    _bind() {
        ...
        this._onMouseDown = (e) => {
            ...
            if (e.button === 0) {
                ...
                if (this.locked || this.photoHeld || this.rightDown) this.shutterPressed = true;
            }
