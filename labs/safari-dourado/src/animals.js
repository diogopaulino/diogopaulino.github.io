...
export function buildGiraffe(BABYLON, scene) {
    const { root, plane } = createBillboard(BABYLON, scene, 'giraffe', '/labs/safari-dourado/assets/giraffe.webp', 4.5, 6.0, 0);
    return { root, body: plane, species: 'giraffe', name: 'Girafa-da-savana' };
}

export function buildElephant(BABYLON, scene) {
    const { root, plane } = createBillboard(BABYLON, scene, 'elephant', '/labs/safari-dourado/assets/elephant.webp', 5.5, 4.0, 0);
    return { root, body: plane, species: 'elephant', name: 'Elefante-africano' };
}

export function buildZebra(BABYLON, scene) {
    const { root, plane } = createBillboard(BABYLON, scene, 'zebra', '/labs/safari-dourado/assets/zebra.webp', 3.0, 2.5, 0);
