(() => {
  ...
  function noise(duration, volume, when = 0, highpass = 150) {
    ...
    source.buffer = buffer; source.connect(filter); filter.connect(gainAt(volume, start, duration));
    source.start(start);
  }
  ...
  function startMusic(stage) {
    ...
    tick(); musicTimer = setInterval(tick, stage === 'coliseum' ? 180 : 210);
  }

  function stopMusic() { if (musicTimer) clearInterval(musicTimer); musicTimer = null; }
  function setMuted(value) { muted = value; localStorage.setItem('rk-muted', value ? '1' : '0'); if (master) master.gain.setTargetAtTime(value ? 0 : .58, context.currentTime, .02); }

  window.RKAudio = { ensure, sfx, startMusic, stopMusic, setMuted, get muted() { return muted; } };
})();
